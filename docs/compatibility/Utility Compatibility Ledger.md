...
| `VER` | No stock CP/M counterpart | `BASIC.CPX` plus matching `VER.COM` fallback | Concise system release; `/V` reports every completed subsystem identity, API version, and implementation version | CPX and transient implementations complete; core copy remains temporarily | Physical concise/verbose CPX and transient tests; runtime values are generated from the normative TSV matrix |
