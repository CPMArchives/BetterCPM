...
A host-based cross-assembler may be supported as a development convenience and independent verification path. It shall not be required to build BetterCP/M, and canonical source shall not depend upon host-only syntax or features. Every operating-system binary produced by both native and cross build paths shall be byte-identical. An automated comparison shall enforce this invariant and any mismatch shall fail the build. Listings, symbol tables, and maps may differ in representation, but they shall describe the same addresses and emitted bytes. Canonical operating-system binaries shall not embed timestamps, host paths, tool-version strings, or other build-environment metadata that would defeat reproducibility.
