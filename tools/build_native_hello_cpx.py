#!/usr/bin/env python3
"""Build HELLO.CPX code under native CP/M and require cross parity."""
...
def main() -> None:
    ...
    for path in (args.cpmsim, args.system_disk, args.disk_template,
                 ...
                 SOURCE, BUILD / "hello.bin"):
        if not path.is_file():
            raise SystemExit(f"missing native HELLO.CPX build input: {path}")
    with tempfile.TemporaryDirectory(prefix="bettercpm-native-hello-cpx-") as temporary:
        work = Path(temporary)
        disks = work / "disks"
        disks.mkdir()
...
send -- "B:\r"
expect "B>"
send -- "D:ZSM4 B:HELLO=C:HELLO\r"
expect -re {{Errors: +0}}
expect "B>"
send -- "D:LINK HELLO\\[A\\]\r"
expect "CODE SIZE"
expect "B>"
send "\034"
expect eof
'''
        ...
        (BUILD / "NATIVE-HELLO-CPX-BUILD.LOG").write_text(
            transcript, encoding="utf-8")
        if (result.returncode or "Errors: 0" not in transcript
                or "CODE SIZE" not in transcript):
            raise SystemExit(f"native HELLO.CPX build failed\n{transcript}")
        native_com = work / "HELLO.COM"
        run("cpmcp", "-f", "ibm-3740", str(disks / "driveb.dsk"),
            "0:HELLO.COM", str(native_com))
        cross = (BUILD / "hello.bin").read_bytes()
        native = native_com.read_bytes()[:len(cross)]
        if native != cross:
            raise SystemExit(
                f"native/cross HELLO.CPX mismatch: linked size {native_com.stat().st_size}")
        (BUILD / "hello-native.bin").write_bytes(native)
        print(f"HELLO.CPX: {len(native)} byte-identical bytes")
