...
def main():
    ...
    for base in (0x8000,0xB103):
        ...
        for name,binding in records():
            ...
            for track in sorted(set([0,len(order)//2-1,len(order)//2,len(order)-1])):
                ...
                for record in (0, (secs<<size)-1):
                    cpu.bc=0x1B7;cpu.de=track;cpu.hl=record
                    ...
                    cases+=1
            cpu.bc=0x1B7;cpu.de=len(order);cpu.hl=0
            cpu.run(base+4)
        ...
        for sizes in ([0,3,1,2], [3,3,3,3,3,2], [2,0,2,1,3,0]):
            ...
            for index,size in enumerate(sizes):
                for quarter in range(1<<size):
                    cpu.bc=0x1B7;cpu.de=2;cpu.hl=record
        ...
        cpu.ix=0x9000;cpu.mem[0x9002]+=1;cpu.bc=0x2B7
        cpu.run(base+4)
        assert cpu.a==255
        ...
        cpu.mem[0x7100:0x710C]=bytes([2,2,0,0])+b'FDF     '
        cpu.bc=177;cpu.de=0x7100
        cpu.run(base+4)
