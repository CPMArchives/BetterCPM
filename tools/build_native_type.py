#!/usr/bin/env python3
"""Build TYPE.COM under native CP/M and require cross-build parity."""
...
def main() -> None:
    ...
    for path in (args.cpmsim, args.system_disk, args.disk_template,
                 ...
                 SOURCE, BUILD / "TYPE.COM"):
        if not path.is_file():
            raise SystemExit(f"missing native-build input: {path}")

    with tempfile.TemporaryDirectory(prefix="bettercpm-native-type-") as temporary:
        work = Path(temporary)
        disks = work / "disks"
        disks.mkdir()
...
send -- "B:\r"
expect "B>"
send -- "D:ZSM4 B:TYPE=C:TYPE\r"
expect -re {{Errors: +0}}
expect "B>"
send -- "D:LINK TYPE\\[A\\]\r"
expect "CODE SIZE"
expect "B>"
send "\034"
expect eof
'''
        ...
        (BUILD / "NATIVE-TYPE-BUILD.LOG").write_text(transcript,
                                                      encoding="utf-8")
        if result.returncode or "Errors: 0" not in transcript or "CODE SIZE" not in transcript:
            raise SystemExit(f"native TYPE build failed\n{transcript}")
        native_com = work / "TYPE.COM"
        run("cpmcp", "-f", "ibm-3740", str(disks / "driveb.dsk"),
            "0:TYPE.COM", str(native_com))
        cross = (BUILD / "TYPE.COM").read_bytes()
        native = native_com.read_bytes()[:len(cross)]
        (BUILD / "TYPE-native.COM").write_bytes(native)
        if native != cross:
            raise SystemExit(f"native/cross TYPE mismatch: linked size {native_com.stat().st_size}")
        print(f"TYPE: {len(native)} byte-identical bytes")
