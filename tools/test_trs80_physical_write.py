#!/usr/bin/env python3
"""Boot a disposable image and verify physical write/readback."""
import subprocess
import tempfile
from pathlib import Path

from build_montezuma_extended_790k import RAW_SIZE, SECTOR_SIZE, TRACK_DATA_SIZE, build
from build_trs80_boot import SOURCE, BUILD, assemble
...
EXPECTED = b"BIOS physical write verified"


def main() -> None:
    boot = assemble(ASSEMBLER, SOURCE / "boot.mac", BUILD / "boot.bin", 0x4300)
    test = assemble(ASSEMBLER, SOURCE / "diskwrit.mac", BUILD / "diskwrit.bin", 0x5000)
    ...
    raw[track + 9 * SECTOR_SIZE:track + 9 * SECTOR_SIZE + 16] = b"original-sector!"
    image = BUILD / "BetterCPM-physical-write-test.dmk"
    image.write_bytes(build(bytes(raw)))
    with tempfile.TemporaryDirectory(prefix="bettercpm-physical-write-") as temporary:
        subprocess.run([str(EMULATOR), "-m4", "-batch", "-turbo", "-d0", str(image),
                        "-id", "240", "-it", "-ix"], cwd=temporary, check=True)
        screen = Path(temporary, "trs80-text-0.bin").read_bytes()
    if not screen.startswith(EXPECTED):
        raise SystemExit(f"physical-write test failed: {screen[:80]!r}")
    print(EXPECTED.decode("ascii"))
    print("cylinder 2, side 1, sector 10 write/readback passed")
