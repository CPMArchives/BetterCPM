#!/usr/bin/env python3
"""Build WARM.COM under native CP/M and require cross parity."""
...
SOURCE = ROOT / "src/utilities/warm.mac"
...
def main() -> None:
    ...
    for path in (args.cpmsim, args.system_disk, args.disk_template,
                 ...
                 SOURCE, BUILD / "WARM.COM"):
        if not path.is_file():
            raise SystemExit(f"missing native WARM build input: {path}")
    with tempfile.TemporaryDirectory(prefix="bettercpm-native-warm-") as temporary:
        work = Path(temporary)
        disks = work / "disks"
        disks.mkdir()
...
send -- "B:\r"
expect "B>"
send -- "D:ZSM4 B:WARM=C:WARM\r"
expect -re {{Errors: +0}}
expect "B>"
send -- "D:LINK WARM\\[A\\]\r"
expect "CODE SIZE"
expect "B>"
send "\034"
expect eof
'''
        ...
        (BUILD / "NATIVE-WARM-BUILD.LOG").write_text(transcript, encoding="utf-8")
        if result.returncode or "Errors: 0" not in transcript or "CODE SIZE" not in transcript:
            raise SystemExit(f"native WARM build failed\n{transcript}")
        native_com = work / "WARM.COM"
        run("cpmcp", "-f", "ibm-3740", str(disks / "driveb.dsk"),
            "0:WARM.COM", str(native_com))
        cross = (BUILD / "WARM.COM").read_bytes()
        native = native_com.read_bytes()[:len(cross)]
        if native != cross:
            raise SystemExit(f"native/cross WARM mismatch: linked size {native_com.stat().st_size}")
        (BUILD / "WARM-native.COM").write_bytes(native)
        print(f"WARM: {len(native)} byte-identical bytes")
