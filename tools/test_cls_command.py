#!/usr/bin/env python3
"""Verify the resident CLS command on the Model 4 console."""
...
def main() -> None:
    for path in (DEFAULT_EMULATOR, IMAGE):
        if not path.is_file():
            raise SystemExit(f"missing CLS test input: {path}")
    with tempfile.TemporaryDirectory(prefix="bettercpm-cls-") as temporary:
        disk = Path(temporary, IMAGE.name)
        disk.write_bytes(IMAGE.read_bytes())
        invocation = [str(DEFAULT_EMULATOR), "-m4", "-batch", "-turbo",
                      "-d0", str(disk), "-id", "3000"]
        for command in ("VER", "CLS"):
    ...
    if b"BetterCP/M 0.3" in screen or b"A0>CLS" in screen:
        raise SystemExit(f"CLS left old screen contents visible: {screen!r}")
    if b"A0>" not in screen:
        raise SystemExit(f"CLS did not leave a fresh prompt: {screen!r}")
    print("CLS cleared the Model 4 display and left a fresh prompt")
