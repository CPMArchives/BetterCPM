#!/usr/bin/env python3
"""Build the resident gateway under native CP/M and require cross parity."""
...
def main() -> None:
    ...
    for path in (args.cpmsim, args.system_disk, args.disk_template,
                 ...
                 SOURCE, BUILD / "gateway.bin"):
        if not path.is_file():
            raise SystemExit(f"missing native-build input: {path}")

    with tempfile.TemporaryDirectory(prefix="bettercpm-native-system-") as temporary:
        work = Path(temporary)
        disks = work / "disks"
        disks.mkdir()
...
send -- "B:\r"
expect "B>"
send -- "D:ZSM4 B:GATEWAY=C:GATEWAY\r"
expect -re {{Errors: +0}}
expect "B>"
send -- "D:LINK GATEWAY\\[A\\]\r"
expect "CODE SIZE"
expect "B>"
send "\034"
expect eof
'''
        ...
        (BUILD / "NATIVE-GATEWAY-BUILD.LOG").write_text(transcript,
                                                         encoding="utf-8")
        if result.returncode or "Errors: 0" not in transcript or "CODE SIZE" not in transcript:
            raise SystemExit(f"native gateway build failed\n{transcript}")
        native_com = work / "GATEWAY.COM"
        run("cpmcp", "-f", "ibm-3740", str(disks / "driveb.dsk"),
            "0:GATEWAY.COM", str(native_com))
        cross = (BUILD / "gateway.bin").read_bytes()
        native = native_com.read_bytes()[:len(cross)]
        if native != cross:
            raise SystemExit(f"native/cross gateway mismatch: linked size {native_com.stat().st_size}")
        (BUILD / "gateway-native.bin").write_bytes(native)
        print(f"GATEWAY: {len(native)} byte-identical bytes")
