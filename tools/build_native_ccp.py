#!/usr/bin/env python3
"""Build the resident CCP under native CP/M and require cross parity."""
...
def main() -> None:
    ...
    for path in (args.cpmsim, args.system_disk, args.disk_template,
                 ...
                 SOURCE, BUILD / "ccp.bin"):
        if not path.is_file():
            raise SystemExit(f"missing native-build input: {path}")

    with tempfile.TemporaryDirectory(prefix="bettercpm-native-ccp-") as temporary:
        work = Path(temporary)
        disks = work / "disks"
        disks.mkdir()
...
'''
        ...
        (BUILD / "NATIVE-CCP-BUILD.LOG").write_text(transcript,
                                                     encoding="utf-8")
        if result.returncode or "Errors: 0" not in transcript or "CODE SIZE" not in transcript:
            raise SystemExit(f"native CCP build failed\n{transcript}")
        native_com = work / "CCP.COM"
        run("cpmcp", "-f", "ibm-3740", str(disks / "driveb.dsk"),
            "0:CCP.COM", str(native_com))
        cross = (BUILD / "ccp.bin").read_bytes()
        native = native_com.read_bytes()[:len(cross)]
        if native != cross:
            raise SystemExit(f"native/cross CCP mismatch: linked size {native_com.stat().st_size}")
        (BUILD / "ccp-native.bin").write_bytes(native)
        print(f"CCP: {len(native)} byte-identical bytes")
