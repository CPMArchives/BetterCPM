#!/usr/bin/env python3
"""Build ERA.COM under native CP/M and require cross-build parity."""
...
SOURCE = ROOT / "src/utilities/era.mac"
...
def main() -> None:
    ...
    for path in (args.cpmsim, args.system_disk, args.disk_template,
                 ...
                 SOURCE, BUILD / "ERA.COM"):
        if not path.is_file():
            raise SystemExit(f"missing native-build input: {path}")

    with tempfile.TemporaryDirectory(prefix="bettercpm-native-era-") as temporary:
        work = Path(temporary)
        disks = work / "disks"
        disks.mkdir()
...
'''
        ...
        (BUILD / "NATIVE-ERA-BUILD.LOG").write_text(transcript,
                                                     encoding="utf-8")
        if result.returncode or "Errors: 0" not in transcript or "CODE SIZE" not in transcript:
            raise SystemExit(f"native ERA build failed\n{transcript}")
        native_com = work / "ERA.COM"
        run("cpmcp", "-f", "ibm-3740", str(disks / "driveb.dsk"),
            "0:ERA.COM", str(native_com))
        cross = (BUILD / "ERA.COM").read_bytes()
        native = native_com.read_bytes()[:len(cross)]
        (BUILD / "ERA-native.COM").write_bytes(native)
        if native != cross:
            raise SystemExit(f"native/cross ERA mismatch: linked size {native_com.stat().st_size}")
        print(f"ERA: {len(native)} byte-identical bytes")
